...
* 智能后期清理：
    - 黑名单过滤：支持关键词与扩展名双模式匹配，自动删除命中的广告、说明文件（如 .url, .txt, 扫码 等）。
...
* 清理防重入：当已有清理任务执行时，后续触发会被自动跳过，避免重叠执行。
...
services:
  cd2-bot:
    ...
    environment:
      ...
      - MAX_REPORT_LINES=60                 # 清理报告最大展示条数
...
| MAX_REPORT_LINES | 否  | 60 | /clean 返回报告最多展示条数，超出部分折叠 |
| PROXY_URL      | 否  | - | 连接 Telegram 的代理，支持 http/socks5 |
| CLEAN_CRON     | 否  |  30 3 * * * | 定时清理任务的 Cron 表达式|

### 黑名单匹配规则

* 以 `.` 开头的词（如 `.txt`、`.url`）会按“扩展名”精确匹配。
* `txt`、`url`、`md`、`nfo` 会自动按扩展名处理（等价于 `.txt`、`.url`、`.md`、`.nfo`）。
* 其他词按“文件名包含关键词”匹配。

> 建议：优先使用扩展名规则，避免使用过短关键词造成误删。
